...
tmp.library.list <- c("haven", "zoo", "fUnitRoots", "tseries", "urca", "lmtest", "forecast", "data.table", "readxl","reshape", "quantmod", "ggplot2", "reshape2", "plyr","scales", "hts", "fpp2", "lubridate","stargazer","GGally", "dplyr")
...
ggsave("./Charts_deseas/EDA_correlation.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/total_cases_distribution.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/time_series_totalcases.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/time_series_logtotalcases_maxtemp.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/time_series_logtotalcases_humidity.png", width = 8, height = 6)
```
...
ggsave("./Charts_deseas/ACF.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/PACF.png", width = 8, height = 6)
...
```{r}
# Fit Neural Network
nn.fit4 <- nnetar(sjtrain.ts,xreg=sjtrain.ts1[,5:23])
fit4
```
...
decomp = stl(sjtest.ts1, s.window="periodic")
plot(decomp)
sjtest.ts1 <- seasadj(decomp)
...
ggsave("./Charts_deseas/ARIMA vs. Ground Truth.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/SARIMA vs. Ground Truth.png", width = 8, height = 6)
...
ggplot() + 
  geom_line(mapping=aes(x=week_start_date, y=total_cases, color="ori"), data=sjdata.test) + 
  geom_line(mapping=aes(x=sjdata.test$week_start_date, y=fc2$mean, color="pred")) + 
  theme_light() + theme(legend.position = "right") + 
  ggtitle("SARIMAX Lag 4 vs. Ground Truth")

ggsave("./Charts_deseas/SARIMAX Lag 4 vs. Ground Truth.png", width = 8, height = 6)
...
ggplot() + 
  ...
  theme_light() + theme(legend.position = "right") + 
  ggtitle("SARIMAX Lag 1 vs. Ground Truth")

ggsave("./Charts_deseas/SARIMAX Lag 1 vs. Ground Truth.png", width = 8, height = 6)
...
ggsave("./Charts_deseas/Neural networks prediction vs. Ground Truth.png", width = 8, height = 6)
